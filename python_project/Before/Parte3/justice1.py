...
for i in range(len(data1)):
    ...
    if t2 == 160 or t2 == 185:
        ...
        if t2 == 185:
            array_real = np.array(array_count1, dtype=float)
            data_array_provavel = pd.DataFrame({"Arrays": array_provavel})

            for k, name in enumerate(data_array_provavel['Arrays']):
                name = np.array(name, dtype=float)
                print(array_real, name)
                if np.array_equal(name, array_real):    #    sucessos, n = media, len(array_real)
                    print('opa')
                    time.sleep(5000)
                else:
                    print("Não foi encontrado array")
                    time.sleep(6000)
