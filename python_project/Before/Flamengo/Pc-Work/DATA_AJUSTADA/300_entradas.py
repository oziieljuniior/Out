...
# Inicializar classes
lautgh1 = np.zeros(300, dtype = int)
lautgh2 = np.zeros(300, dtype = int)
...
while i <= 210000:
    ...
    if len(media_parray) < 299:
        m = 0
    else:
        m = media_parray[len(media_parray) - 300]
    ...
    if i <= inteiro:
        ...
        if i >= 300:
            ...
            if (i % 300) == 0 and i > 300:
                print('*-'*16)
                print(f'LArrayII: {[len(array3n[i - 600: i - 300]), len(array3s[i - 600: i - 300])]}')
                if i == 600:
                    matrix1s = np.array(array3s[i - 600: i - 300]).reshape(-1,1)
                    matrix1n = np.array(array3n[i - 600: i - 300]).reshape(-1,1)
            
                else:
                    array3ss = np.array(array3s[i - 600: i - 300]).reshape(-1,1)
                    array3ns = np.array(array3n[i - 600: i - 300]).reshape(-1,1)
