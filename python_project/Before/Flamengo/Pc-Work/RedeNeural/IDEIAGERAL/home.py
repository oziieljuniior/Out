...
# Inicializar classes
lautgh1 = np.zeros(120, dtype = int)
lautgh2 = np.zeros(120, dtype = int)
    ...
    if i <= inteiro:
        ...
        if i >= 120:
            ...
            if (i % 120) == 0 and i > 120:
                print('*-'*16)
                print(f'LArrayII: {[len(array3n[i - 240: i - 120]), len(array3s[i - 240: i - 120])]}')
                if i == 240:
                    matrix1s = np.array(array3s[i - 240: i - 120]).reshape(-1,1)
                    matrix1n = np.array(array3n[i - 240: i - 120]).reshape(-1,1)
            
                else:
                    array3ss = np.array(array3s[i - 240: i - 120]).reshape(-1,1)
                    array3ns = np.array(array3n[i - 240: i - 120]).reshape(-1,1)
