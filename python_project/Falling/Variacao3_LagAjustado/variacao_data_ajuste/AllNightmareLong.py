    ...
    while True:
        try:
            if i <= inteiro:
                ...
                odd = float(input("Insira o número da odd: ").replace(",","."))
                break
        except ValueError:
...
## Verificação das ultimas entradas com modelos já salvos;
            while True:
                try:
                    ...
                    if pergunta1 == 0:
                        kil1, kil2 = [], []
                        ajuste = [x - 0.5 for x in data_teste]
                        # Ajustar o modelo AR ao array2
                        if i <= 330:
                            guitar = [1,20]
                            model_ar = AutoReg(ajuste, lags=guitar)  # Ajuste o lag conforme 
                        if i > 330 and i <= 630:
                            guitar = [1,60,61]
                            model_ar = AutoReg(ajuste, lags=guitar)  # Ajuste o lag conforme model_ar = AutoReg(data_teste, lags=guitar)  # Ajuste o lag conforme sua análise de ACF
                        if i > 630 and i <= 930:
                        ...
                        ajuste1 = [x + 0.5 for x in previsao_ar]
