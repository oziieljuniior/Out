#Ajustar código para produção, no caso, adicionar funcionalidade do inteiro
...
array_acuracia = []
t = 2*10**5
limite = int(input("Insira até o modelo deve ser carregado: "))

while i <= t:
    if i >= len(data):
        break

    array_float, array_binario, odd = coletarodd(i, limite, data, array_float, array_binario)

    if odd == 0:
        break
    print(36*'*')
    print(f'Rodada: {i} | Odd: {odd}')
