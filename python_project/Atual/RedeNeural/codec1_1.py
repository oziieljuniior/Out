...
def reden(array1, array2):
    ...
    model.compile(
        loss=tf.keras.losses.BinaryFocalCrossentropy(alpha=0.85, gamma=2.0),
...
while i <= 210000:
    ...
    if i >= 360 and (i % 60) == 0:
        matriz_final_float, matriz_final_int = vetores.tranforsmar_final_matriz(array2s)
        modelo = reden(matriz_final_float, matriz_final_int)    

    if i >= 360:
        print(24*'*-')
        Apredicao = vetores.transformar_entrada_predicao(array2s)
        #print(f'Predição: {type(Apredicao)} | {len(Apredicao)}')
        res = modelo.predict(Apredicao)
        if res[0] == 1:
            resultado = 0
        else:
            resultado = 1
