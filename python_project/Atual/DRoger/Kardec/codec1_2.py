...
# Threshold dinâmico baseado na proporção desejada de 1s
def aplicar_threshold_dinamico(probs, proporcao=0.3):
    k = int(len(probs) * proporcao)
    top_k = np.argsort(probs.flatten())[-k:]
    resultado = np.zeros_like(probs)
    resultado[top_k] = 1
    return resultado

# Função principal
def reden(array1, array2, proporcao_1=0.3):
    """
    Treina uma rede neural para detecção da classe 1 em dados sequenciais e desbalanceados.
    """
    X = np.array(array1)
    y = np.array(array2)
    n = X.shape[1]
    ...
    pred_bin = aplicar_threshold_dinamico(probs, proporcao=proporcao_1)
    ...
    plt.plot(pred_bin, label="Previsto", alpha=0.7)
    plt.legend()
...
while i <= 210000:
    ...
    if i >= 360 and (i % 60) == 0:
        ...
        for click in info:
            ...
            #modelo = reden(array1, array2, proporcao_1=0.3)
            models = reden(array1, array2, proporcao_1=0.3)
