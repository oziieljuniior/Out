...
class AjustesOdds:
    ...
    def tranforsmar_final_matriz(self, array1):
        ...
        # 2. Matriz majorada
        array_majorado = [40 if v <= 40 else 60 if v >= 60 else v for v in array1]
        ...
        x17, x18 = self.gerar_direcionalidade(array2, limite=100, inverter=False)

        # 6. Direcionalidade 2 (<= 1.02)
        x19, x20 = self.gerar_direcionalidade(array2, limite=25, inverter=True)
        ...
        matrizX_final = np.concatenate((x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x17, x18, x19, x20), axis=1)

        array1binario1 = [0 if odd >= 50 else 1 for odd in array1]
    ...
    def transformar_entrada_predicao(self, array1):
        ...
        # 2. Array majorado
        array1marjorado = [
            40 if val <= 40 else 60 if val >= 60 else val
            for val in array1
        ]
        ...
        x17, x18 = self.processa_direcao(array2, limite=100, inverter=False)
        x19, x20 = self.processa_direcao(array2, limite=25, inverter=True)
        ...
        matrizX_final = np.concatenate((x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x17, x18, x19, x20), axis=0)
